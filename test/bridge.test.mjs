...
test('MCP initialization and listing do not connect or submit',async()=>{assert.equal((await rpc({jsonrpc:'2.0',id:1,method:'initialize'})).result.serverInfo.name,'codex-dsh-mcp');assert.equal((await rpc({jsonrpc:'2.0',id:2,method:'tools/list'})).result.tools.length,8);});
