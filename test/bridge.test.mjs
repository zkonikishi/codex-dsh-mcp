...
test('MCP initialization and listing do not connect or submit',async()=>{assert.equal((await rpc({jsonrpc:'2.0',id:1,method:'initialize'})).result.serverInfo.name,'codex-dsh-mcp');assert.equal((await rpc({jsonrpc:'2.0',id:2,method:'tools/list'})).result.tools.length,5);});
...
test('same request replay never resubmits; different payload collides',async()=>{
 const dir=await mkdtemp('D:/Servers/AI/Data/Temp/dsh-mcp-test-');let sent=0;
...
test('running session never receives prompt',async()=>{
 const dir=await mkdtemp('D:/Servers/AI/Data/Temp/dsh-mcp-test-');try{assert.equal((await submit({stateDirectory:dir},{sessionId:'s',requestId:'r',text:'work'},async()=>({items:[{sessionId:'s',running:true}]}))).state,'SESSION_RUNNING');}finally{await rm(dir,{recursive:true});}
});
test('concurrent requests to same session cannot both submit',async()=>{
 const dir=await mkdtemp('D:/Servers/AI/Data/Temp/dsh-mcp-test-');let release;const gate=new Promise(r=>release=r);let entered;const ready=new Promise(r=>entered=r);
